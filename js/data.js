let data = [
    {active: true, name: 'Главная', id: 'main', content: mainContent},
    {active: false, name: 'О нас', id: 'about', content: aboutContent},
    {active: false, name: 'Услуги и цены', id: 'price', content: priceContent},
    {active: false, name: 'Акции', id: 'stocks', content: stocksContent},
    {active: false, name: 'Врачи', id: 'personal', content: personalContent},
    {active: false, name: 'Вакансии', id: 'job', content: jobContent},
]